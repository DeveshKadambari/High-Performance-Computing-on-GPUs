...
__global__ void gpu_transpose_mult(int *a,int *b, int *c, int m, int n, int k)
{
  __shared__ int smem[512];
    ...
    for(int i=(index_begin);i<index_end;i++)                            //Row=0->80
        { 	for(int k_l=0;k_l<512;K_l++){
		smem[tid] =a[i * (blockDim.x)+tid] *b[k_l*(blockDim.x)+tid];     //h_a[row_variable*Max_column + column_variable]//save multiplication value into the smem buffer
		tid+=256;
		smem[tid] =a[i * (blockDim.x)+tid] *b[k_l*(blockDim.x)+tid];
         __syncthreads();                                               //wait until all the threads reach this point
                for(int j=((n)/2);j>0;j=ceilf(f))            //i=250/2 is 125->62.5(63)->(63-1)->(21)->20->(10)->(5)->(3)->(2)->(1)
                { k_b=2*f;
